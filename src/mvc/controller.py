...
class Controller:
    ...
    def handle_menu_button(self, choice: str):
        if choice == "XML":
            ...
            try:
                ...
                if __paths:
                    XML2Img(
                        __paths[0],
                        __paths[1],
                        self.__model.get_class_name(),
                        self.__model.get_class_color(),
                        self.__model.get_class_counter(),
                    )
            ...
            try:
                ...
                if __path:
                    ...
                    self.show_info_dialog(
                        "System Info", f"Done splitting {__imgset.total} labels"
                    )
            except AttributeError:
                self.show_error_dialog("System Error", "No Folder Path!")
            ...
            c = CaptureImg(
                self.__model.get_index(),
                (self.__model.get_width(), self.__model.get_height()),
            )
            c.run()
            ...
            try:
                ...
                if __path:
                    __file_changer = FileChange(
                        __path[0] + "{}", __path[1], self.__model.get_file_counter()
                    )
                    __file_changer.run()
                    self.show_info_dialog(
                        "System Info", f"Done Changed {__file_changer.file} files"
                    )
            except AttributeError:
                self.show_error_dialog("System Error", "No Folder Path!")
