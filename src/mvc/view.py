    ...
    def popup_component(self, GUI):
        ...
        for choice in choices:
            button = Button(
                ...
                bg="black",
                fg="white",
                command=lambda c=choice: self.button_click(c),
            )
            button.pack(pady=2)  # Add space between buttons

            # Bind mouse enter event for hover effect
            button.bind(
                "<Enter>", lambda event, b=button: b.config(bg="green", fg="white")
            )

            # Bind mouse leave event for normal state
            button.bind(
                "<Leave>", lambda event, b=button: b.config(bg="black", fg="white")
            )
