...
class YOLO2Img:
    ...
    def yolo_to_opencv(self, x, y, width, height, image_width, image_height):
        x_min, y_min = int((x - width / 2) * image_width), int(
            (y - height / 2) * image_height
        )
        x_max, y_max = int((x + width / 2) * image_width), int(
            (y + height / 2) * image_height
        )
            ...
            try:
                with open(yolo_file, "r") as label:
                    ...
                    sys.stdout.write(
                        ...
                        + "." * progress
                        + " " * (40 - progress)
                        + f"] {self.__image_counter}/{self.total_image}"
                    )
                    sys.stdout.flush()

                    for label_line in label_lines:
                        data = label_line.strip().split(" ")
                        class_id, x_coor, y_coor, width, height = (
                            int(data[0]),
                            float(data[1]),
                            float(data[2]),
                            float(data[3]),
                        ...
                        x_min, y_min, x_max, y_max = self.yolo_to_opencv(
                            x_coor, y_coor, width, height, image_width, image_height
                        )
    ...
    def __class_plt(self):
        __class_colors_rgb = [
            self.__bgr_to_rgb(bgr_color) for bgr_color in self.__class_colors
        ]
