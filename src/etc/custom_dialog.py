...
class CustomDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        tk.Label(master, text='Select label:').grid(row=1)
        ...
        self.__button_data_browse = tk.Button(master, text="Browse", command=lambda:self.__select_folder('data'))
        self.__button_data_browse.grid(row=0, column=2, padx=5)

        self.__button_label_browse = tk.Button(master, text="Browse",command=lambda:self.__select_folder('label'))
